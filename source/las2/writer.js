            ...
            if (!project) { //export from inventory
				...
				if (curve.type == 'ARRAY') {
					for (let i = 0; i < curve.dimension; i++) {
						fs.appendFileSync(lasFilePath, space.spaceAfter(WHLEN1, normalizedCurveName + `[${i}]`) + space.spaceAfter(WHLEN2 + WHLEN3, '.' + curve.curve_revisions[0].unit) + ': ' + curve.description + '\r\n');
					}
				} else {
